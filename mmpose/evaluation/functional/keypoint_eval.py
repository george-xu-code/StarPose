...
def simcc_pck_accuracy(output: Tuple[np.ndarray, np.ndarray],
                       ...
                       x_simcc_split_ratio: float,
                       y_simcc_split_ratio: float,
                       mask: np.ndarray,
                       thr: float = 0.05,
                       normalize: Optional[np.ndarray] = None) -> tuple:
    """Calculate the pose accuracy of PCK for each individual keypoint and the
    ...
    N, _, Wx = pred_x.shape
    _, _, Wy = pred_y.shape
    W, H = int(Wx / x_simcc_split_ratio), int(Wy / y_simcc_split_ratio)

    if normalize is None:
        normalize = np.tile(np.array([[H, W]]), (N, 1))

    pred_coords, _ = get_simcc_maximum(pred_x, pred_y)
    pred_coords_split = np.split(pred_coords, 2, axis=2)
    x_pred_coords = pred_coords_split[0] / x_simcc_split_ratio
    y_pred_coords = pred_coords_split[1] / y_simcc_split_ratio
    pred_coords = np.concatenate((x_pred_coords, y_pred_coords), axis=2)

    gt_coords, _ = get_simcc_maximum(gt_x, gt_y)
    gt_coords_split = np.split(gt_coords, 2, axis=2)
    x_gt_coords = gt_coords_split[0] / x_simcc_split_ratio
    y_gt_coords = gt_coords_split[1] / y_simcc_split_ratio
    gt_coords = np.concatenate((x_gt_coords, y_gt_coords), axis=2)

    return keypoint_pck_accuracy(pred_coords, gt_coords, mask, thr, normalize)


def simcc_pck_accuracy_before(output: Tuple[np.ndarray, np.ndarray],
                              target: Tuple[np.ndarray, np.ndarray],
